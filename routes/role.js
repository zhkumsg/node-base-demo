...
router.get("/", function (req, res, next) {
    ...
    if (!Routebase.IsPermit(req, res, "00026")) { return; }
    switch (req.query["method"]) {
        case "Backend_GetRoleList": GetRoleList(req, res); break;
        case "Backend_GetRoleDetail": GetRoleDetail(req, res); break;
...
router.post("/", function (req, res, next) {
    ...
    if (!Routebase.IsPermit(req, res, "00026")) { return; }
    switch (req.query["method"]) {
        case "Backend_DeleteRole": DeleteRole(req, res); break;
        case "Backend_InsertRole": InsertRole(req, res); break;
        case "Backend_UpdateRole": UpdateRole(req, res); break;
        default: res.json(MsgJsonHelper.DebugJson("接口请求错误")); break;
    }
});
...
function GetRoleList(req, res) {
    ...
    Routebase.ValidRole(req.UserInfo.ZK_ROLE).then(roles => {
        ...
        let keys = [];
        roles.forEach(r => { keys.push(r.ZK_ID); });
        condition.push(new MemoryCondition({
            Field: "ZK_ID",
            Logic: MLogic.And,
        ...
        client.Query(QueryModel.ZK_ROLEINFO, condition, null, Number.parseInt(pagesize, 10), Number.parseInt(pageno, 10), true, sort).then(m => {
...
function GetRoleDetail(req, res) {
    ...
    client.Query(QueryModel.ZK_ROLEINFO, condition, null, 0, 0, false, new SortParam()).then(m => {
        if (m.result.length > 0) {
...
function InsertRole(req, res) {
    ...
    ds.GetTable("select * from ZK_ROLEINFO where (ZK_ROLE = " + ds.parse(record.ZK_ROLE) + " OR ZK_LEAVE = " + record.ZK_LEAVE + ") and EB_ISDELETE = '0'").then(dt => {
        if (dt.length > 0) {
...
function UpdateRole(req, res) {
    ...
    if (record.ZK_ID === "00001") { delete record.ZK_LEAVE; }
    ds.TransRunQuery(Public.OperationSQLParams(record, OperationEnum.UpdateNoCheck)).then(flag => {
        res.json(MsgJsonHelper.DefaultJson(null, flag, flag ? "保存成功" : "保存失败，请检查数据后重新提交"));
    }).catch(err => {
