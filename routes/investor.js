...
router.get('/', function(req, res, next) {
	...
	if (!Routebase.IsPermit(req, res, '00052')) {
		return;
	}
	switch (req.query['method']) {
		case 'Backend_GetInvestorList':
			GetInvestorList(req, res);
			break;
		case 'Backend_GetInvestorDetail':
			GetInvestorDetail(req, res);
			break;
		case 'Backend_GetTargetDetail':
			GetTargetDetail(req, res);
			break;
		default:
...
router.post('/', function(req, res, next) {
	...
	switch (req.query['method']) {
		case 'Backend_DeleteInvestor':
			DeleteInvestor(req, res);
			break;
		case 'Backend_UpdateInvestor':
			UpdateInvestor(req, res);
			break;
		case 'Backend_SyncTarget':
			SyncTarget(req, res);
			break;
		default:
			res.json(MsgJsonHelper.DebugJson('接口请求错误'));
...
function GetInvestorDetail(req, res) {
	...
	client
		.Query(QueryModel.ZK_INVESTOR, [condition[0]], null, 0, 0, false, new SortParam())
		.then(m1 => {
			if (m1.result.length > 0) {
				client
					.Query(QueryModel.ZK_INVESTOR_CASE, [condition[1]], null, 0, 0, false, new SortParam())
					.then(m2 => {
						client
							.Query(QueryModel.ZK_INVESTOR_TEAM, [condition[1]], null, 0, 0, false, new SortParam())
							.then(m3 => {
								res.json(
...
function GetTargetDetail(req, res) {
	...
	client
		.Query(QueryModel.ZK_INVESTOR, condition, null, 0, 0, false, new SortParam())
		.then(m => {
			if (m.result.length > 0) {
				let name = m.result[0].ZK_TITLE;
				new MySqlHelper()
					.GetTable("select * from mc_investor_org where status = 1 and name like '%" + name + "%'")
					.then(dt => {
						let result = [];
						if (dt.length > 0) {
							dt.forEach(item => {
