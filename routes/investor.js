...
function UpdateInvestor(req, res) {
  ...
  record.EB_LASTMODIFY_DATETIME = Public.StringToDate(
    req.body["EB_LASTMODIFY_DATETIME"]
  );
  ...
  ds.TransRunQuery(Public.OperationSQLParams(record, OperationEnum.Update))
    .then(flag => {
      if (flag) {
        ...
        res.json(
          MsgJsonHelper.DebugJson("修改失败，请刷新页面重新提交或检查数据")
        );
      }
    })
    .catch(err => {
  ...
  client
    ...
    .then(m => {
      if (m.result.length > 0) {
        ...
        new MySqlHelper()
          .GetTable(
            "select * from mc_investor_org where status = 1 and name like '%" +
              name +
              "%'"
          )
          .then(dt => {
