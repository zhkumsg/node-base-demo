...
router.get('/', function(req, res, next) {
	...
	if (!Routebase.IsPermit(req, res, '00028')) {
		return;
	}
	switch (req.query['method']) {
		case 'Backend_GetPermitList':
			...
			break;
		case 'Backend_GetPermitDetail':
			GetPermitDetail(req, res);
			break;
		default:
...
router.post('/', function(req, res, next) {
	...
	switch (req.query['method']) {
		case 'Backend_DeletePermit':
			DeletePermit(req, res);
			break;
		case 'Backend_InsertPermit':
			InsertPermit(req, res);
			break;
		case 'Backend_UpdatePermit':
			UpdatePermit(req, res);
			break;
		default:
			res.json(MsgJsonHelper.DebugJson('接口请求错误'));
			break;
...
function GetPermitDetail(req, res) {
	...
	client
		.Query(QueryModel.ZK_PERMITINFO, condition, null, 0, 0, false, new SortParam())
		.then(m => {
			if (m.result.length > 0) {
