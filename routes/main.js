...
router.post("/", function (req, res, next) {
    ...
    if (!Routebase.IsPermit(req, res, "00002")) { return; }
    switch (req.query["method"]) {
        case "reptile_investment": ReptileInvestment(req, res); break;
        case "reptile_investor": ReptileInvestor(req, res); break;
...
function ReptileInvestment(req, res) {
    ...
    res.json(MsgJsonHelper.DefaultJson(null, true, "程序启动成功，正在爬取投融事件信息（可在投融事件页面查看最新数据）"));
}
...
function ReptileInvestor(req, res) {
    ...
    res.json(MsgJsonHelper.DefaultJson(null, true, "程序启动成功，正在爬取投资机构信息（可在投资机构页面查看最新数据）"));
}
