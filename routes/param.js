...
router.get("/", function (req, res, next) {
    ...
    switch (req.query["method"]) {
        case "Backend_GetParamList": GetParamList(req, res); break;
        case "Backend_GetParamDetail": GetParamDetail(req, res); break;
...
router.post("/", function (req, res, next) {
    ...
    if (!Routebase.IsPermit(req, res, "00017")) { return; }
    switch (req.query["method"]) {
        case "Backend_DeleteParams": DeleteParams(req, res); break;
        case "Backend_InsertParam": InsertParam(req, res); break;
        case "Backend_UpdateParam": UpdateParam(req, res); break;
        default: res.json(MsgJsonHelper.DebugJson("接口请求错误")); break;
    }
});
...
function GetParamList(req, res) {
    ...
    client.Query(QueryModel.ZK_PARAMINFO, condition, null, Number.parseInt(pagesize, 10), Number.parseInt(pageno, 10), true, sort).then(m => {
    ...
    client.DeleteByIds(ZK_PARAMINFO, ids.split(",")).then(m => {
        if (m) {
            global["SYS_PARAMINFO"].forEach(p => {
                if (ids.indexOf(p.ZK_ID) > -1) { p.EB_ISDELETE = "1"; }
            });
...
function GetParamDetail(req, res) {
    ...
    client.Query(QueryModel.ZK_PARAMINFO, condition, null, 0, 0, false, new SortParam()).then(m => {
        if (m.result.length > 0) {
...
function InsertParam(req, res) {
    ...
    ds.GetTable("select * from ZK_PARAMINFO where ZK_KEY = " + ds.parse(record.ZK_KEY)).then(dt => {
        if (dt.length > 0) {
            ...
            ds.TransRunQuery(Public.OperationSQLParams(record, OperationEnum.Create)).then(flag => {
                if (flag) { global["SYS_PARAMINFO"].push(record); }
...
function UpdateParam(req, res) {
    ...
    ds.GetTable("select * from ZK_PARAMINFO where ZK_KEY = " + ds.parse(record.ZK_KEY) + " and ZK_ID !=" + ds.parse(record.ZK_ID)).then(dt => {
        if (dt.length > 0) {
            res.json(MsgJsonHelper.DebugJson("键要保证全局唯一，不能重复，请修改数据"));
        } else {
            ds.TransRunQuery(Public.OperationSQLParams(record, OperationEnum.Update)).then(flag => {
                if (flag) { global["SYS_PARAMINFO"].push(record); }
                res.json(MsgJsonHelper.DefaultJson(null, flag, flag ? "更新成功" : "更新失败，请检查数据后重新提交"));
            }).catch(err => {
