...
function InsertParam(req, res) {
  ...
  ds.GetTable(
    "select * from ZK_PARAMINFO where ZK_KEY = " + ds.parse(record.ZK_KEY)
  )
    .then(dt => {
      if (dt.length > 0) {
        res.json(
          MsgJsonHelper.DebugJson("键要保证全局唯一，不能重复，请修改数据")
        );
      } else {
        ds.TransRunQuery(
  ...
  ds.GetTable(
    ...
    .then(dt => {
      if (dt.length > 0) {
        ...
        ds.TransRunQuery(
          Public.OperationSQLParams(record, OperationEnum.Update)
        )
          .then(flag => {
            if (flag) {
              global["SYS_PARAMINFO"].push(record);
