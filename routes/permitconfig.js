...
router.get("/", function (req, res, next) {
    ...
    if (!Routebase.IsPermit(req, res, "00021")) { return; }
    switch (req.query["method"]) {
        case "Backend_BindRoleList": BindRoleList(req, res); break;
        case "Backend_BindPermitList": BindPermitList(req, res); break;
...
router.post("/", function (req, res, next) {
    ...
    switch (req.query["method"]) {
        ...
        case "Backend_UpdatePermitConfig": UpdatePermitConfig(req, res); break;
        default: res.json(MsgJsonHelper.DebugJson("接口请求错误")); break;
    }
});
...
function BindRoleList(req, res) {
    ...
    client.Query(QueryModel.ZK_ROLEINFO, condition, null, 0, 0, false, sort).then(m => {
...
function GetPermitList(req, res) {
    ...
    client.Query(QueryModel.ZK_PERMITCONFIG, condition, null, 0, 0, false, new SortParam()).then(m => {
        if (m.result.length > 0) {
            res.json(MsgJsonHelper.DefaultJson(m.result, true, m.recordcount.toString()));
        }
        else {
...
function UpdatePermitConfig(req, res) {
    ...
    if (permits.length > 0) {
        permits.forEach(item => {
            sqlstr.push("INSERT INTO ZK_PERMITCONFIG (ZK_ID, ZK_PERMITID, ZK_ROLE) VALUES(" + ds.parse(Public.BuildCode()) + "," + ds.parse(item) + "," + ds.parse(role) + ")");
        });
    }
    ds.RunQuery(sqlstr.join(";")).then(flag => {
        if (flag) {
