...
router.get("/", function (req, res, next) {
  ...
  if (!Routebase.IsPermit(req, res, "00005")) { return; }
  switch (req.query["method"]) {
    case "Backend_GetUserList": GetUserList(req, res); break;
    case "Backend_GetUserInfo": GetUserInfo(req, res); break;
...
router.post("/", function (req, res, next) {
  ...
  if (!Routebase.IsPermit(req, res, "00005")) { return; }
  switch (req.query["method"]) {
    case "Backend_DeleteUser": DeleteUser(req, res); break;
    case "Backend_InsertUser": InsertUser(req, res); break;
    case "Backend_UpdateUser": UpdateUser(req, res); break;
    default: res.json(MsgJsonHelper.DebugJson("接口请求错误")); break;
  }
});
...
function GetUserList(req, res) {
  ...
  Routebase.ValidRole(req.UserInfo.ZK_ROLE).then(roles => {
    ...
    client.Query(QueryModel.ZK_USERINFO, condition, null, Number.parseInt(pagesize, 10), Number.parseInt(pageno, 10), true, sort).then(m => {
...
function GetUserInfo(req, res) {
  ...
  client.Query(QueryModel.ZK_USERINFO, condition, null, 0, 0, false, new SortParam()).then(m => {
    if (m.result.length > 0) {
...
function UpdateUser(req, res) {
  ...
  ds.TransRunQuery(Public.OperationSQLParams(user, OperationEnum.UpdateNoCheck)).then(flag => {
