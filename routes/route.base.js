...
module.exports = {
  ...
  IsLogin(req, res) {
    if (req["UserInfo"]) {
      ...
      res.json(
        MsgJsonHelper.DefaultJson(null, false, "登录超时", JsonCodeEnum.Timeout)
      );
      return false;
    }
  },
  ...
  ValidRole(code) {
    return new Promise(resolve => {
      let sqlstr =
        "SELECT ZK_ID, ZK_ROLE FROM ZK_ROLEINFO WHERE EB_ISDELETE = '0' AND ZK_LEAVE >= (SELECT ZK_LEAVE FROM ZK_ROLEINFO WHERE ZK_ID = " +
        ds.parse(code) +
        ")";
      ds.GetTable(sqlstr)
        .then(dt => {
